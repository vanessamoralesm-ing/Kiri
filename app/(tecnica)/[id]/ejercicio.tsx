...
export default function EjercicioTecnica() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const [tecnicaObtenida, pasosObtenidos] = await Promise.all([
        obtenerTecnica(idTecnica),

        obtenerPasosTecnica(idTecnica),
      ]);

      if (!pasosObtenidos.length) {
        throw new Error("Esta técnica todavía no tiene pasos disponibles.");
      }

      setTecnica(tecnicaObtenida);

      setPasos(pasosObtenidos);

      setIndice(0);
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "No pudimos preparar el ejercicio.",
      );
    } finally {
      setCargando(false);
    }
  ...
  const siguiente = async () => {
    ...
    if (!idRegistro) {
      Alert.alert("No pudimos guardar", "Falta el registro de esta práctica.");

      return;
    }

    if (!idTecnica) {
      Alert.alert("No pudimos finalizar", "No se encontró la técnica actual.");
