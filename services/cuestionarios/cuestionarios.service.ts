...
export interface OpcionRespuestaTest {
    ...
    // Incluye la puntuación inversa si ya está
    // configurada en opcion_test.valor_puntaje.
    valor: number | null;
...
export type ResultadoGuardado = Pick<
    ResultadoTest,
...
export async function obtenerUsuarioCuestionario(): Promise<UsuarioCuestionario> {
    const {
        data: { user },
        error: authError,
    ...
    if (!user) {
        throw new Error("Debes iniciar sesión para responder este cuestionario.");
    }
    ...
    if (!data) {
        throw new Error("No fue posible encontrar tu perfil de usuario.");
    }
...
export async function obtenerOCrearEjecucion(
    ...
    if (errorExistente) {
        console.error("Error al buscar ejecución:", errorExistente);
    ...
    if (errorNueva) {
        console.error("Error al crear ejecución:", errorNueva);
        throw errorNueva;
    }
...
export async function completarEjecucion(idEjecucion: string): Promise<void> {
    const { data, error } = await supabase
        .from("ejecucion_test")
        .update({
            estado: "completado",
    ...
    if (!data) {
        throw new Error("No se encontró una ejecución en progreso para completar.");
    }
}
...
export async function obtenerSubescalasPorTest(
    ...
    if (error) {
        console.error("Error al obtener subescalas:", error);
        throw error;
    }
...
export async function obtenerPreguntasConOpciones(
    ...
    return (data ?? []).map((pregunta): PreguntaTestConOpciones => {
        const opcionesDB = (pregunta.opcion_test ?? []) as OpcionTest[];

        const opciones = opcionesDB
            .filter((opcion) => opcion.estado)
            .sort((a, b) => a.orden - b.orden)
            .map((opcion): OpcionRespuestaTest => ({
                id: opcion.id_opcion,
                codigo: opcion.codigo,
                texto: opcion.etiqueta,

                // No invertir nuevamente:
                // utilizar el valor registrado.
                valor:
                    opcion.valor_puntaje === null ? null : Number(opcion.valor_puntaje),
        ...
        return {
            ...
            id_subescala: pregunta.id_subescala,
            codigo: pregunta.codigo,
            enunciado: pregunta.enunciado,
            descripcion_apoyo: pregunta.descripcion_apoyo,
            tipo_pregunta: pregunta.tipo_pregunta as PreguntaTest["tipo_pregunta"],
            orden: pregunta.orden,
            obligatoria: pregunta.obligatoria,
            puntua: pregunta.puntua,
            es_observacional: pregunta.es_observacional,
            permite_comentario: pregunta.permite_comentario,
            estado: pregunta.estado,
            fecha_creacion: pregunta.fecha_creacion,
            fecha_actualizacion: pregunta.fecha_actualizacion,
            opciones,
        };
    });
}
...
export async function obtenerRespuestasEjecucion(idEjecucion: string) {
    const { data, error } = await supabase
        .from("respuesta_test")
        .select(
            `
...
export async function guardarRespuestaTest({
    ...
    const { error } = await supabase.from("respuesta_test").upsert(
        {
        ...
        {
            onConflict: "id_ejecucion,id_pregunta",
        },
    );
...
export async function obtenerBaremosPorTest(
    ...
    if (error) {
        console.error("Error al obtener baremos:", error);
        throw error;
    }
...
export async function guardarResultadoTest({
    ...
    if (!Number.isFinite(puntajeDirecto) || !Number.isFinite(puntajeTotal)) {
        throw new Error(
            "No se puede guardar un resultado con puntuaciones inválidas.",
        );
    }

    if (idRangoBaremo && !idBaremo) {
        throw new Error("No se puede asociar un rango sin su baremo.");
    }
        ...
        .upsert(
            {
                ...
                observaciones,
                id_baremo: idBaremo,
                id_rango_baremo: idRangoBaremo,
                tipo_finalizacion: "completa",
            },
            {
                onConflict: "id_ejecucion",
            },
        )
        .select(
            `
...
export async function guardarResultadoSubescala({
    ...
    if (!Number.isFinite(puntajeDirecto)) {
        throw new Error("La puntuación de la subescala no es válida.");
    }

    const { error } = await supabase.from("resultado_subescala").upsert(
        {
            id_resultado: idResultado,
            id_subescala: idSubescala,
            puntaje_directo: puntajeDirecto,
            puntaje_transformado: null,
            nivel_cualitativo: nivel,
            interpretacion_texto: interpretacion,
        },
        {
            onConflict: "id_resultado,id_subescala",
        },
    );

    if (error) {
        console.error("Error al guardar resultado de subescala:", error);
        throw error;
    }
}
...
export async function obtenerTestsConConteoPreguntas(): Promise<
...
> {
    ...
    if (error) {
        console.error("Error al obtener tests con conteo de preguntas:", error);
        throw error;
    }

    return (data ?? []) as TestConConteoPreguntas[];
}
